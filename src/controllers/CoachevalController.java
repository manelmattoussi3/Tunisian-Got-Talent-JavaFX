 ...
 */
public class CoachevalController implements Initializable {
    ...
    @FXML
    private TableColumn<Rate,String> avis;
    ...
    private void btnCandidatAction(ActionEvent event) throws IOException {
     ...
     public void loadScreenButton() {
        try {
            ...
            ResultSet rs = conn.createStatement().executeQuery("select R.id,C.nom as nom_candidat ,F.description as formation,R.avis from rating as R,Candidat as C ,Formation as F where R.Candidat_id=C.id AND R.Formation_id=F.id");
            ...
            while (rs.next()) {
                data.add(new Rate(rs.getInt(1),rs.getString(2), rs.getString(3), rs.getString(4)));
